...
1. If found, substitute the token's value.
1. If NOT found, the processor MUST use a platform-appropriate default
...
1. **`fallback` arrays** — ordered fallback chains when a widget is
   unavailable.
...
1. Validate it against the Formspec Theme JSON Schema
   (`theme.schema.json`).
1. Verify that `$formspecTheme` is a supported version.
1. Reject the theme if any REQUIRED property is missing.
...
1. **Check compatibility** — verify target Definition match.
1. **Resolve tokens** — collect all `$token.` references in `style`
   and `widgetConfig` values. Substitute each with the corresponding
   token value. Unresolved tokens use platform defaults.
1. **For each item** in the Definition:
...
1. **Compute layout** — if `pages` is present, assign items to pages
   and regions. Apply responsive overrides based on the current
   viewport.
1. **Emit resolved presentation** — the final per-item presentation
...
1. Tier 1 inline `presentation` hints on each item (core §4.2.5).
1. Renderer platform defaults.
