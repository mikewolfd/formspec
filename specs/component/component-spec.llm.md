...
1. **Input and Display** components MUST NOT have a `children` property.
   If present, processors MUST reject the document or ignore the
   `children` property and emit a warning.

1. **Wizard** children MUST all be `Page` components. A processor MUST
   reject a Wizard whose children contain non-Page components.

1. **Spacer** MUST NOT have children (it is a Layout leaf).

1. Nesting depth SHOULD NOT exceed 20 levels. Processors MAY reject
...
1. **Hint:** The renderer SHOULD display the item's `hint` when available.
1. **Required indicator:** The renderer MUST indicate required state when
   the item's Bind `required` expression evaluates to `true`.
1. **Read-only state:** The renderer MUST disable editing when the item's
   Bind `readOnly` expression evaluates to `true` or when the item has
   a `calculate` Bind.
1. **Relevant state:** When the item's Bind `relevant` expression
   evaluates to `false`, the renderer MUST hide the component. This
   operates independently of the component's `when` property (§8.2).
1. **Validation errors:** The renderer MUST display validation results
...
1. Within each repeat instance, resolve child `bind` values relative
   to the repeat context. Child keys are still flat item keys, but they
   resolve within the current repeat instance.
1. Provide affordances for adding and removing repeat instances, subject
...
1. The renderer MUST render a fallback input for each unbound required
   item.
1. Fallback rendering MUST use Tier 2 theme rules if a Theme Document
   is present, or Tier 1 `presentation` hints otherwise, or renderer
   defaults as a last resort.
1. Fallback inputs MUST be appended **after** the component tree's
   rendered output, in Definition document order.
1. The renderer SHOULD visually distinguish fallback-rendered items
...
1. The `when` property (transferred to the fallback component).
1. The `responsive` property (transferred if applicable props exist
   on the fallback).
1. The `style` property.
1. The `bind` property (when the fallback supports it).
...
1. `{{` produces a literal `{` in the output.
1. `}}` produces a literal `}` in the output.
1. Nesting is NOT allowed: `{outer_{inner}}` is invalid.
1. An unrecognized `{name}` (where `name` is not in the template's
...
1. Verify that all declared params have corresponding values in the
   instantiation's `params` object. Missing params MUST cause a
   validation error.
1. Deep-clone the template's `tree`.
1. Replace all `{paramName}` occurrences in allowed string props
   with the corresponding values from `params`.
1. Insert the resolved subtree in place of the custom component
   reference.
...
1. The processor MUST evaluate the expression against the current
   data tree.
1. If the expression evaluates to `true`, the component (and all
   its children) MUST be rendered.
1. If the expression evaluates to `false`, `null`, or any non-boolean
...
1. The processor MUST emit a warning identifying the component and
   the expression error.
1. The processor MUST NOT halt form rendering due to a `when`
   evaluation error.
...
1. The component is hidden (null → falsy).
1. The processor SHOULD emit a warning.
...
1. **Base props** apply to all viewport widths (including the
   smallest).
1. At each breakpoint (in ascending min-width order), the
   corresponding override object is **shallow-merged** on top of
   the base props.
1. Each override replaces individual props; it does not deep-merge
...
1. The `children` array MUST NOT change per breakpoint (no adding,
   removing, or reordering children).
1. The `bind` property MUST NOT change per breakpoint.
...
1. If NOT found, look up `key` in the Theme Document's `tokens`.
1. If NOT found, use the renderer's default value.
...
1. The processor SHOULD emit a warning identifying the unresolved
   token reference.
1. The processor MUST NOT fail or halt rendering.
...
1. **Tier 1 (Definition hints):** If no Theme applies, use the
   item's `presentation` hints (core §4.2.5) to select a widget
   and configure rendering.
1. **Renderer defaults:** If no hints are available, use the
...
1. Identify all Definition items not bound in the tree.
1. Render unbound visible items using fallback rules, appended after
   the tree output.
1. Ensure all required items are editable, regardless of whether
...
1. **Type correctness:** Each property has the correct JSON type
   (string, object, array, integer, boolean) as specified.
1. **Enum constraints:** Properties with enumerated values
   (`direction`, `align`, `severity`, `position`, etc.) contain
   valid values.
1. **Component names:** Every `component` value is either a built-in
   name (§5, §6) or a key in the `components` registry.
1. **Children constraints:** Components that do not accept children
   (§3.4) do not have a `children` property.
1. **ConditionalGroup `when`:** ConditionalGroup components include
   a `when` property.
1. **Wizard children:** All children of a Wizard are Page components.
1. **Heading props:** `level` is 1–6 and `text` is present.
...
1. **Token references:** Every `$token.key` reference SHOULD resolve
...
1. **Custom component references:** Every `component` value that is
   not a built-in name MUST exist as a key in the `components`
   registry. References to undefined custom components MUST be
   rejected.

1. **Custom component params:** When instantiating a custom component,
...
1. **Summary and DataTable bind refs:** The `bind` values within
   Summary `items` and DataTable `columns` MUST reference valid
   item keys.

1. **Cycle-free custom components:** The custom component dependency
...
1. Check the component's compatible dataTypes list.
1. If the dataType is NOT in the list, emit a validation error.
...
1. **Custom style keys:** Style object keys starting with `x-` are
   vendor-specific. Processors MUST ignore unrecognized `x-` style
   keys.

1. **Custom token prefixes:** Token keys starting with `x-` follow
