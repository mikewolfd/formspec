...
1. **Index bounds:** An explicit index `$repeat[n]` where `n < 1` or
   `n >` the number of instances MUST signal an evaluation error.
1. **Instance lookup:** `@instance('name')` MUST match a declared Data Source.
   If the named instance does not exist, the processor MUST signal a
   definition error.
1. **Chaining:** Dot-segments after an indexed or wildcard subscript continue
...
1. **MUST** accept all input strings that match the `Expression` production of
   this grammar.
1. **MUST** reject all input strings that do not match the `Expression`
   production. Rejection MUST include a diagnostic indicating the approximate
   position of the syntax error.
1. **MUST** treat whitespace and comments as insignificant except inside string
   literals.
1. **MUST** implement all escape sequences defined in §3.4. An unrecognised
   escape sequence MUST be rejected as a syntax error.
1. **MUST** enforce the reserved-word restriction in §3.3: reserved words MUST
   NOT be accepted as function names.
1. **MUST** parse the `|>` (pipe) character sequence as a syntax error in
   v1.0. This token is reserved for future use.
1. **SHOULD** produce a parse tree (or equivalent AST) that preserves the
