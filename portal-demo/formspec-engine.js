...
import { interpreter } from './fel/interpreter';
...
export class FormEngine {
    constructor(definition) {
        ...
        this.structureVersion = signal(0);
    ...
    interpolateMessage(message, contextPath) {
    ...
    initItem(item, prefix = '') {
        const key = item.key;
        if (!key)
            throw new Error("Item missing required 'key'");
        ...
        if (item.type === 'group') {
            ...
            let initialValue = item.initialValue !== undefined ? item.initialValue : '';
            const dataType = item.dataType;
            if (!dataType)
                throw new Error(`Field '${fullName}' missing required 'dataType'`);
            ...
            if (bind) {
                ...
                if (bind.required) {
                    if (typeof bind.required === 'string') {
                        const compiled = this.compileFEL(bind.required, fullName, undefined, true);
                ...
                if (bind.readonly) {
                    if (typeof bind.readonly === 'string') {
                        const compiled = this.compileFEL(bind.readonly, fullName, undefined, true);
    ...
    initRepeatInstance(item, fullName, index) {
        if (!item.children)
            return;
    ...
    compileFEL(expression, currentItemName, index, includeSelf = false) {
        ...
        const compiled = () => {
            ...
            for (const dep of astDeps) {
                ...
                if (this.signals[fullDepPath])
                    this.signals[fullDepPath].value;
                if (this.repeats[fullDepPath])
                    this.repeats[fullDepPath].value;
                if (this.relevantSignals[fullDepPath])
                    this.relevantSignals[fullDepPath].value;
            }
            const context = {
                getSignalValue: (path) => this.signals[path]?.value,
                getRepeatsValue: (path) => this.repeats[path]?.value ?? 0,
                getRelevantValue: (path) => this.relevantSignals[path]?.value ?? true,
                getRequiredValue: (path) => this.requiredSignals[path]?.value ?? false,
                getReadonlyValue: (path) => this.readonlySignals[path]?.value ?? false,
