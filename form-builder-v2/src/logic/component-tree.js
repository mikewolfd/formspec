...
export function classifyNode(node) {
    ...
    if (LAYOUT_COMPONENTS.has(component))
        return 'layout';
    if (INPUT_COMPONENTS.has(component) && bind)
        return 'bound-input';
    if (DISPLAY_COMPONENTS.has(component) && bind)
...
export function nodeKindColor(kind) {
    switch (kind) {
...
// --- Tree navigation ---
export function resolveNode(tree, path) {
    if (path === '')
        return tree;
...
// --- Label resolution ---
export function getNodeLabel(node, definitionItems) {
...
function findItemByKeyFlat(key, items) {
...
export function generateComponentTree(definition) {
...
export function findPathByBind(tree, bind, currentPath = '') {
    if (tree.bind === bind)
        return currentPath;
    if (tree.children) {
...
export function acceptsChildren(componentType) {
