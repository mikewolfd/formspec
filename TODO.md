...
- **Shared cross-seam fixture suite** `[6 / 5 / 4]` (**24**) — end-to-end fixtures exercising the five canonical seams (canonical response, governance coprocessor, event hash chain, checkpoint seal, custody hook). The WOS-T4 cross-stack signature fixture should be the first real slice of this suite, not a parallel one-off corpus. STACK.md flags as open gap: "composition correctness is asserted by prose." When the suite lands, claims stop depending on prose. **Gate: none — design + scaffold.**
