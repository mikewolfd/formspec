...
export const DragHandle = forwardRef<Element, DragHandleProps>(function DragHandle(
  ...
  return (
    <button
      ...
      aria-label={label}
      tabIndex={-1}
      className={`self-stretch flex items-center justify-center opacity-[0.45] group-hover:opacity-100 group-focus-within:opacity-100 transition-[opacity,color,background-color] cursor-grab shrink-0 rounded-[6px] px-1 text-ink/45 hover:bg-bg-default/55 hover:text-ink/70 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/35 ${className}`}
    >
