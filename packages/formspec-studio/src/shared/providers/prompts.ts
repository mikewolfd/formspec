import type { AnalysisV1, InquestModelInput, InquestTemplate, ProposalV1 } from '../contracts/inquest';
...
function buildAnalysisUserPrompt(input: InquestModelInput): string {
  const parts: string[] = [];
  ...
  const userMessages = input.session.input.messages
  ...
  if (input.session.input.uploads.length > 0) {
    parts.push(`\n## Uploaded Files\n`);
    for (const upload of input.session.input.uploads) {
      parts.push(`- ${upload.name} (${upload.mimeType}, ${upload.size} bytes)`);
      if (upload.excerpt) {
        parts.push(`  Excerpt: ${upload.excerpt}`);
      }
    }
  }
...
function buildProposalUserPrompt(input: InquestModelInput, analysis: AnalysisV1): string {
  const parts: string[] = [];

  parts.push(`## Session Context`);
  parts.push(`Title: ${input.session.title}`);
  parts.push(`Session URL: urn:formspec:inquest:${input.session.sessionId}`);
...
Set form presentation. Payload: \`{ properties: { pageMode?: string, labelPosition?: string, density?: string } }\`
...
Create/update a named option set. Payload: \`{ name: string, options: Array<{ value: string, label: string }> }\`
...
function buildEditUserPrompt(input: InquestModelInput): string {
  const parts: string[] = [];
...
const CHAT_SYSTEM = `
You are Stack Assistant, an AI helper for building forms with Formspec.
