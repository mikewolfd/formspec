...
- **schemaValidator** (`SchemaValidator`): Optional schema validator (from formspec-engine createSchemaValidator). When set, diagnose() runs structural validation and populates the structural diagnostics array. Omit in environments where schemas are not available (e.g. browser without bundled schemas).
...
- **signature** (`string`): Function signature (e.g. `'sum(array) -> number'`).
