...
} from '../src/providers/deterministic';
import type {
  AnalysisV1,
  InquestModelInput,
  InquestSessionV1,
...
describe('buildEditPatch', () => {
  ...
  describe('add X field', () => {
    ...
    it('infers money type for amount-related field names', () => {
      ...
      expect((result.commands[0].payload as Record<string, unknown>).dataType).toBe('money');
    });

    it('infers date type for date-related field names', () => {
      const result = buildEditPatch(makeInput({ prompt: 'add start date field' }));

      expect((result.commands[0].payload as Record<string, unknown>).dataType).toBe('date');
    });
  });
