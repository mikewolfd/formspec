...
import { useInquestOps } from '../src/hooks/useChatOps';
import type {
  InquestProviderAdapter,
  InquestSessionV1,
  InquestTemplate,
} from 'formspec-shared';
import { buildAnalysis, buildProposal } from '../src/providers';
import { InquestDraft } from '../src/authoring/draft';
...
describe('useInquestOps — handleAnalyze early-return guard', () => {
  ...
  it('DOES call the provider when description is empty but a template is selected', async () => {
    ...
    const template: InquestTemplate = {
      ...
      tags: [],
      starterPrompts: [],
      seedAnalysis: { fields: [{ key: 'name', label: 'Name', dataType: 'string', required: true }], sections: [], rules: [], repeats: [], routes: [] },
    };
