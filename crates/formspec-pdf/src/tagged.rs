...
pub struct FieldTag {
    ...
    /// Tooltip text for /TU (from label or hint).
    #[allow(dead_code)]
    pub tooltip: String,
}

/// A struct element in registration order (label or field).
enum Tag {
    Label(usize),
    Field(usize),
}
...
pub struct TaggingContext {
    ...
    /// Tags in registration order for correct reading order in the structure tree.
    tags: Vec<Tag>,
...
impl TaggingContext {
    ...
    /// Set the annotation key offset so StructParent keys don't collide with
    /// page entries in the ParentTree. Call after determining page_count but
    /// before any field tagging.
    pub fn set_annotation_key_offset(&mut self, page_count: usize) {
        self.next_struct_parent = page_count as i32;
    }
    ...
    pub fn write_structure_tree(&self, pdf: &mut Pdf, page_refs: &[Ref], page_count: usize) -> Ref {
        ...
        // Write struct elements in registration order (interleaved labels and fields)
        for tag in &self.tags {
            match tag {
                Tag::Label(idx) => {
                    let lt = &self.label_tags[*idx];
                    let page_ref = page_refs[lt.page_index.min(page_refs.len() - 1)];
                    let mut elem = pdf.struct_element(lt.struct_elem_ref);
                    elem.kind(StructRole::P);
                    elem.parent(lt.parent_ref);
                    elem.page(page_ref);
                    elem.marked_content_child()
                        .page(page_ref)
                        .marked_content_id(lt.mcid);
                    elem.finish();
                }
                Tag::Field(idx) => {
                    let ft = &self.field_tags[*idx];
                    let page_ref = page_refs[ft.page_index.min(page_refs.len() - 1)];
                    let mut elem = pdf.struct_element(ft.struct_elem_ref);
                    elem.kind(StructRole::Form);
                    elem.parent(ft.parent_ref);
                    elem.page(page_ref);
                    elem.object_child().page(page_ref).object(ft.annot_ref);
                    elem.finish();
                }
            }
        }

        // Write Sect element — children in registration order
        {
            ...
            for tag in &self.tags {
                match tag {
                    Tag::Label(idx) => {
                        children.struct_element(self.label_tags[*idx].struct_elem_ref);
                    }
                    Tag::Field(idx) => {
                        children.struct_element(self.field_tags[*idx].struct_elem_ref);
                    }
                }
            }
            children.finish();
        ...
        // Write ParentTree as a NumberTree
        {
            ...
            // Annotation entries: keys already offset by page_count.
            for tag in &self.field_tags {
                nums.item(tag.struct_parent_key);
